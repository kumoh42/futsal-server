...
export class MembersService {
    ...
    async getMembersInfo() {
        const membersWithCircleAndMajor = await this.memberRepo
            ...
            .innerJoin('xe_reservation_circle', 'circle', 'member.circle_srl = circle.circle_srl')
            .innerJoin('xe_reservation_major', 'major', 'member.major_srl = major.major_srl')
    ...
    async changeMembersInfo(memberSrl: number, changedInfo: MemberInfoDto){
        ...
        const member = await this.memberRepo
                                    ...
                                    .getOne();
if(!member){ throw new NotFoundException('해당 사용자가 존재하지 않습니다.')};
    ...
    async deleteMember(memberSrl: number){
        ...
        if (result.affected === 0) {
            throw new NotFoundException(`${memberSrl} 에 해당하는 srl 이 존재하지 않습니다.`);
        }
        return ['삭제 완료'];
    }
    ...
    async approveMember(memberSrl: number){ 
        ...
        .createQueryBuilder('member')
        .where('member.member_srl = :memberSrl', { memberSrl })
        .andWhere('member.is_denied = :isDenied', { isDenied: 'N' })
        .getOne();
        if(!member){ throw new  BadRequestException('해당 사용자가 존재하지 않거나 권한이 승인되었습니다.')};
        await this.memberRepo
        .createQueryBuilder()
        .update(Xe_Reservation_MemberEntity)
        .set({
